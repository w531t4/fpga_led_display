...
module scan_prefetch (
    ...
    assign wanted_next_row = next_row(row_address_active);
    assign inactive_cache_select = ~active_cache_select;
    assign current_in_active = cache_matches(cache_valid[active_cache_select], cache_row[active_cache_select], row_address_active);
    assign current_in_inactive = cache_matches(cache_valid[inactive_cache_select], cache_row[inactive_cache_select], row_address_active);
    ...
    assign stream_targets_current = stream_inflight_q && (stream_row_q == row_address_active);
        ...
        if (reset) begin
            ...
            if (invalidate_caches) begin
                ...
                if (store_if.prefetch_data_valid && store_if.prefetch_data_ready && store_if.prefetch_data_last) begin
                    stream_inflight_q <= 1'b0;
                    if (stream_row_q == row_address_active) begin
                        activate_cache_select_valid <= 1'b1;
                ...
                if (!request_valid_q && !stream_inflight_q) begin
                    if (!current_in_active && !current_in_inactive) begin
                        ...
                        request_row_q <= row_address_active;
