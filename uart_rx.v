...
global BAUDSET_DATA_INIT
	   BAUDSET_TESTVALS = ["L", "-", "C"]

BAUDSET_DATA_INIT=2000000
BAUDSET_DATA = BAUDSET_DATA_INIT
BAUDSET_DATA_SCALE=1500
...
BAUDSET_DATA_STATE = 1
return
elif BAUDSET_DATA_STATE == 1 and curval == "B":
global BAUDSET_DATA_STATE
BAUDSET_DATA_STATE = 2
return
elif BAUDSET_DATA_STATE == 2 and curval == "C":
global BAUDSET_DATA_STATE
BAUDSET_DATA_STATE = 3
return
elif BAUDSET_DATA_STATE != -1:
...
structure.append({ 'name': 'timeout_word_start', 'size': 1 })
structure.append({ 'name': 'clkdiv_baudrate_reset', 'size': 1 })
...
structure.append({ 'name': 'rx_invalid', 'size': 1 })
...
bytestring = bytestring + ser.read_until()
hexsstrings = ""
binstring = ""
...
if (len(bytestring) > expected_bytes):
stdscr.addstr(0,0, "ERROR: received more than" + str(expected_bytes) + " bits")
else:
stdscr.addstr(0,0, "                                                 ")

#reverse binstring so we can access it sanely using python list constructs
binstring = binstring[::-1]
stdscr.addstr(2,0, "chars read:" + str(len(bytestring)) + " bits:" + str(len(binstring)))
if False:
continue
else:
position = 0
